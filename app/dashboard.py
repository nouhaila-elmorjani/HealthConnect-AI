...
# Batch Analysis Function - FIXED VERSION
def process_batch_data(batch_data, model, feature_names):
    """Process batch data for predictions - FIXED to handle all patients"""
    results = []
...
# Patient Risk Assessment - FIXED BATCH ANALYSIS
                ...
                if st.button("🚀 Run Batch Risk Analysis", type="primary", use_container_width=True):
                    ...
                    if len(results_df) > 0:
                        ...
                        fig = px.pie(
                            ...
                            title=f"Patient Risk Level Distribution ({len(results_df)} patients analyzed)"
                        )
                        st.plotly_chart(fig, use_container_width=True)
